...
void Widget::on_pushButtonAdd_clicked()
{
    ...
    tsLine << strName << "\t" << nAge << "\t"
           << Qt::fixed << qSetRealNumberPrecision(2) << dblWeight;
...
void Widget::on_pushButtonSaveDS_clicked()
{
    ...
    dsOut.setVersion(QDataStream::Qt_6_7);
...
void Widget::on_pushButtonLoadDS_clicked()
{
    ...
    if (!fileIn.open(QIODevice::ReadOnly)) //打开
    {
        //打开失败
    ...
    qint32 nCurAge;
    double dblCurWeight;
    ui->listWidget->clear();//先清空列表控件的内容
    ...
    for (int i = 0; i < nCount; ++i)
    {
        ...
        QString strLine = tr("%1\t%2\t%3")
                          .arg(strCurName)
                          .arg(nCurAge)
                          .arg(dblCurWeight, 0, 'f', 2);//保留小数点后2位
...
void Widget::on_listWidget_currentRowChanged(int currentRow)
{
    ...
    //对于正常的行号
    QString strLine = ui->listWidget->item(currentRow)->text(); //读取该行文本
    QTextStream tsLine(&strLine);//构造内存字符串文本流
    //三个数据项
    QString strName;
    int nAge;
    double dblWeight;
