    ...
    , ui(new Ui::Widget)
{
    ...
    ui->lineEditSrcPort->setValidator( valSrc );//设置验证器
...
void Widget::on_pushButtonSave_clicked()
{
    ...
    {
        //十六进制字符串
        QString strHex = ui->listWidget->item(i)->text();//所有信息的报文
        //转换为原始的数据包
        QByteArray baCur = QByteArray::fromHex(strHex.toUtf8());

        QDataStream dsIn(baCur);//根据字节数组定义输入流 字节数组是只读的
        dsIn >> udpCur; //提取UDP包
...
void Widget::on_pushButtonLoad_clicked()
{
    ...
    for (int i = 0; i < nCount; ++i)
    {
        ...
        dsOut << udpCur; //输出到字节数组

        QString strHex = baCur.toHex(); //把原始字节数组转成十六进制字符串
...
void Widget::on_listWidget_currentRowChanged(int currentRow)
{
    ...
    QDataStream dsIn(baAll);//构造输入流，提取数据
    UDPPacker udp;
