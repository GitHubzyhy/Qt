...
void QwBatery::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
...
QwBatery::QwBatery(QWidget *parent) :
    QWidget(parent)
{
}

void QwBatery::setPowerLevel(int pow)
{
    mPowerLevel = pow;
...
int QwBatery::powerLevel()
{
    return mPowerLevel;
}

void QwBatery::setWarnLevel(int warn)
{
    mWarnLevel = warn;
    repaint();
}

int QwBatery::warnLevel()
{
    return mWarnLevel;
}

QSize QwBatery::sizeHint()
{
    int H = this->height();
    int W = H * 12 / 5;
    QSize size(W, H);
