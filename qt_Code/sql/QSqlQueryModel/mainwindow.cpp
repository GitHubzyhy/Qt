...
void MainWindow::selectData()
{
    ...
    connect(selModel,&QItemSelectionModel::currentRowChanged,
            this,&MainWindow::on_currentRowChange);
...
{//刷新界面
    ...
    selModel->setCurrentIndex(curIndex,QItemSelectionModel::Select);//将模型项索引设置为当前项，并发出currentChanged()
...
{
    QString aFile=QFileDialog::getOpenFileName(this,"选择文件","../","SQLite数据库(*.db *.db3 *.sqlite)");
