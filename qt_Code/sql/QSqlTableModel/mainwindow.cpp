...
void MainWindow::on_actOpenDB_triggered()
{
    ...
    QString aFile=QFileDialog::getOpenFileName(this,"选择文件","../","SQLite数据库(*.sqlite *.db *.db3)");
    if(aFile.isEmpty())
        return;
...
{
    Q_UNUSED(current);Q_UNUSED(previous);//告诉编译器参数没有使用防止出现警告
    //设置项是否可点击
...
{
    //打开一个文件对话框 返回一个完整的路径
    QString aFile=QFileDialog::getOpenFileName(this,"选择一个照片","../","照片(*.jpg *.png *.ico)");
    if(aFile.isEmpty())//如果未空
        return;
    QFile file(aFile);
...
{//降序的单选按钮
    tabModel->sort(ui->comSort->currentIndex(),Qt::DescendingOrder);//按照谁来进行升序
}
