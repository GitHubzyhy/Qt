...
void DragableForm::mousePressEvent(QMouseEvent *event)
{
    ...
    QPixmap pixmap=child->pixmap().scaled(QSize(child->width(),child->height()));//是一个图像表示类
    ...
    painter.end();
    child->setPixmap(tempPixmap);//设置为有个拖动的效果
...
void DragableForm::dragEnterEvent(QDragEnterEvent *event)
{
    ...
    if(event->mimeData()->hasFormat("application/x-dnditemdata"))
    {
        ...
        {
            event->setDropAction(Qt::MoveAction);//将数据从源位置移动到目标位置，
...
void DragableForm::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasFormat("application/x-dnditemdata"))
    {
        ...
        newIcon->move(event->position().toPoint()-offset);//设置label在哪里
        newIcon->show();//显示小部件
