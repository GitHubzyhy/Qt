    ...
    , ui(new Ui::Widget)
{
    ...
    //添加盒饭组合框条目
    ui->comboBoxSnacks->addItem(tr("番茄鸡蛋"), 8.50);//第二个参数是用户数据
    ...
    //同步份数的计数器和滑动条
    connect(ui->spinBoxCount, &QSpinBox::valueChanged,
            ui->horizontalSlider, &QSlider::setValue);
    connect(ui->horizontalSlider, &QSlider::valueChanged,
            ui->spinBoxCount, &QSpinBox::setValue);
}
