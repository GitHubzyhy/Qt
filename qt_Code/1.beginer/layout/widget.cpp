    ...
    , ui(new Ui::Widget)
{
    ...
    qDebug() << tr("Preferred 按钮：")
             << ui->btnPreferred->sizeHint()//推荐尺寸
             << ui->btnPreferred->minimumSizeHint();//最小推荐尺寸

    qDebug() << tr("Expanding 按钮：")
             << ui->btnExpanding->sizeHint()
             << ui->btnExpanding->minimumSizeHint();
...
{
    //通过代码构造第二个示范窗口
    m_pWidget = new QWidget(this, Qt::Window); //设置为独立窗口
...
void Widget::on_btnFixed_clicked()
{
    if (m_pWidget != NULL)
        m_pWidget->show(); //显示
}
