...
{
    //检查姓名 key ，不能为空
    QString strName = ui->lineEditName->text().trimmed(); //剔除空格
    if (strName.isEmpty())
    {
...
{
    int nAge = ui->spinBoxAge->value(); //获取年龄
    //根据年龄反查姓名，可能存在多个同龄人
...
void Widget::showNameAgeMap()
{
    ...
    for (int i = 0; i < nCount; ++i)
    {
        ...
        QString curText = tr("键：%1\t值：%2").arg(listKeys[i]).arg(m_mapNameAge[ listKeys[i] ]);
        //添加到列表控件
