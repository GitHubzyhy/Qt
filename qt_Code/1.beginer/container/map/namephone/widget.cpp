    ...
    , ui(new Ui::Widget)
{
    ...
    QRegularExpression re("[0-9]{3,14}");
    QValidator *validator = new QRegularExpressionValidator(re);
    ui->lineEditPhone->setValidator(validator);
    //结果显示框设为只读
...
void Widget::on_pushButtonAdd_clicked()
{
    ...
    if (strName.isEmpty() || strPhone.isEmpty()) //两个字符串不能为空
    {
...
void Widget::showNamePhoneMap()
{
    ...
    while (it != m_mmapNamePhone.end()) //循环遍历直到结束
    {
        //读取键、值，拼接为字符串
