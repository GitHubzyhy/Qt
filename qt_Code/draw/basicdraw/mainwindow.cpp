...
{
    //在这里用于获取当前选中项的自定义数据。在Qt的数据模型中，每个项都可以关联多个数据，而这些数据可以使用预定义的角色或自定义的角色来标识。
    RenderArea::Shape shape = RenderArea::Shape(ui->shape->itemData(ui->shape->currentIndex(), Qt::UserRole)
                                                    .toInt());//转换为枚举类型
...
void MainWindow::penChanged()
{
    ...
    Qt::PenStyle style = Qt::PenStyle(ui->penStyleComboBox->itemData(
                                                              ui->penStyleComboBox->currentIndex(), Qt::UserRole).toInt());
    //线条端点
...
void MainWindow::brushChanged()
{
  ...
  ui-> brushStyleComboBox->currentIndex(), Qt::UserRole).toInt());
    ...
    if (style == Qt::LinearGradientPattern) {//这里的是线性渐变
        ...
        ui->renderAre->setBrush(QBrush(QPixmap(":/images/brick.png")));
