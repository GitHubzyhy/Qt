    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->View->setCursor(Qt::CrossCursor);//设置光标
...
{//获取鼠标单机事件
    ...
    item=scene->itemAt(pointScene,ui->View->transform());//图形项将用于执行坐标映射。
    if(item!=nullptr)
    {
        QPointF pointItem=item->mapFromScene(pointScene);//将场景坐标映射到图形项的本地坐标系中
