...
{//单机图例的marker的响应
    qDebug()<<"进入点击的槽函数内";
    QLegendMarker *marker=qobject_cast<QLegendMarker *>(sender());//尝试将发送信号的对象转换为 QLegendMarker 类型
...
{//鼠标移动的响应
    QPointF pt=ui->chartView->chart()->mapToValue(point);//转换成图表的数值
