...
//page_3对应的槽函数---------------------------------------------------------------------------------
    ...
    void on_Axis_titleIsVisible_clicked(bool checked);//设置坐标轴标题是否可见
    void on_pBtn_style_clicked();//根据ui界面的lineEdit设置坐标轴标签的格式
    //设置轴刻度颜色和字体
    void on_pBtn_Fontcolor_clicked();
    void on_pBtn_setFont_clicked();

    void on_chk_scaleisVisible_clicked(bool checked);//设置选中坐标轴刻度标签是否可见
    void on_chk_gridLineVisible_clicked(bool checked);//设置选中坐标轴网格线是否可见
    //设置选中坐标轴的网格线的笔或者颜色
    void on_pBtn_gridLineColor_clicked();
    void on_pBtn_gridLinePen_clicked();

    void on_spinBox_Tickcount_valueChanged(int arg1);//设置主刻度线
    void on_spinBox_minorTickcount_valueChanged(int arg1);//设置次刻度线
    void on_chk_TickLineIsVisible_clicked(bool checked);//设置主刻度线是否可见
    void on_pBtn_linePen_clicked();//设置坐标轴线的笔
    void on_pBtn_lineColor_clicked();//设置坐标轴的线条颜色
    void on_MinorTick_Color_clicked();//设置次刻度线对应的网格线颜色
    void on_pBtn_minorLinePen_clicked();//设置次刻度线对应的笔
    void on_chk_MinorTickLineIsVisible_clicked(bool checked);//设置选中坐标轴的次网格线是否可见



    void on_Axis_titlesVisible_clicked(bool checked);
