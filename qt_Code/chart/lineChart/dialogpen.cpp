...
QPen DialogPen::getPen(QPen iniPen, bool &ok)//ok作为判断用户的选择
{
    DialogPen *dlg=new DialogPen;
    QPen pen;
    dlg->setPen(iniPen);//设置笔的颜色并且用笔的颜色填充按钮，目的是打开这个自定义对话框就有初始化效果

    /*将对话框显示为模态对话框，阻塞直到用户关闭它。该函数返回一个DialogCode结果。*/
...
void DialogPen::on_btnColor_clicked()//设置颜色的按钮
{
